...
###
### A simple set of functions to calculate the hydrology
### radius, flow area, and wetted perimeter of a stream 
### channel cross section. This information can be
### supplied from a stream survey or derived from Lidar
### data.  Cross sections data can easily be derived in
##  the HEC-RAS RAS-mapper
###
### Note: - make sure you are using the correct units.
###       - HEC-RAS uses USC (feet) or SI (meters)
#########################################################
...
calculate_cross_sectional_area <- function(
  station,    # vector if station locations across a stream cross section (meters or feet)
  elevation,  # elevation of each stations (meters of feet)
  stage){     # stage depth of the river as related the the thalweg (meters of feet)
...
calculate_wetted_perimeter <- function(
  station,    # vector if station locations across a stream cross section (meters or feet)
  elevation,  # elevation of each stations (meters of feet)
  stage){     # stage depth of the river as related the the thalweg (meters of feet)
...
#                            stage = 10)
# 
# calculate_cross_sectional_area(station = channel.xc.example.data$Station,
#                                elevation = channel.xc.example.data$Elevation,
#                                stage = 10)
# 
# calculate_wetted_perimeter(station = channel.xc.example.data$Station,
#                            elevation = channel.xc.example.data$Elevation,
#                            stage = 10)
# 
# 
